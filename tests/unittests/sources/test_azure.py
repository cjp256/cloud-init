...
def fake_vm_id(monkeypatch):
    vm_id = "fake-vm-id"
    monkeypatch.setattr(identity, "query_vm_id", lambda: vm_id)
    yield vm_id
...
<Msft Virtual Disk 1.0>            at scbus3 target 1 lun 0 (da1,pass2)
    ...
    def test_dsaz_report_failure_appends_supporting_data(self):
        dsrc = self._get_ds({"ovfcontent": construct_ovf_env()})
        ...
        with mock.patch.object(dsrc, "crawl_metadata") as m_crawl_metadata:
            ...
            assert error.supporting_data["vm_id"] == "fake-vm-id"
...
class TestPreprovisioningHotAttachNics(CiTestCase):
    ...
    def test_check_if_nic_is_primary_retries_on_failures(
        ...
        def fake_timer():
            nonlocal fake_time, max_retries
            fake_time += 300 / max_retries
            return fake_time
        ...
        assert len(m_request.mock_calls) == max_retries
