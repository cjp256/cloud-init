...
class TestConvertSystemUuidToVmId:
    ...
    def test_convert_system_uuid_to_vm_id(self, system_uuid, vm_id):
        assert identity.convert_system_uuid_to_vm_id(system_uuid) == vm_id
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "",
                "Failed to parse system uuid '' due to error: "
                "IndexError('list index out of range')",
            ),
            (
                "garbage",
                "Failed to parse system uuid 'garbage' due to error: "
                "ValueError('non-hexadecimal number found in fromhex() "
                "arg at position 0')",
            ),
        ],
    )
    def test_convert_system_uuid_to_vm_id_error(self, system_uuid, error):
        with pytest.raises(RuntimeError) as exc_info:
            identity.convert_system_uuid_to_vm_id(system_uuid)
...
class TestQuerySystemUuid:
    ...
    def test_query_system_uuid(self, mock_read_dmi_data, system_uuid):
    ...
    def test_query_system_uuid_errors(self, mock_read_dmi_data):
