...
class OvfEnvXml:
    def __init__(self, ovf_env_xml: str):
        self._ovf_env_xml = ovf_env_xml
        ...
        try:
            root = ET.fromstring(ovf_env_xml)
        except Exception as e:
    ...
    def _find(
        ...
        if len(matches) == 0:
            msg = "Missing ovf-env.xml configuration for %r" % name
            LOG.debug(msg)
            if error_if_missing:
                raise NonAzureDataSource(msg)
        ...
        if error_if_empty and not matches[0].findall("./"):
            raise BrokenAzureDataSource(
                "Empty configuration in ovf-env.xml for %r" % name
            )
    ...
    def _parse_property(
        ...
        if not matches or len(matches) == 0:
            msg = "Missing ovf-env.xml configuration for property %r" % name
            LOG.debug(msg)
            if error_if_missing:
                raise NonAzureDataSource(msg)
    ...
    def _parse_linux_configuration_set_section(self, provisioning_section):
        ...
        self.custom_data = self._parse_property(
            config_set, "CustomData", decode_base64=True, default=""
        )
        self.username = self._parse_property(config_set, "UserName")
        self.password = self._parse_property(config_set, "UserPassword")
        self.hostname = self._parse_property(config_set, "HostName")
    ...
    def _parse_platform_settings_section(self, root):
        ...
        self.preprovisioned_vm_type = self._parse_property(
            platform_settings, "PreprovisionedVMType", default=None
        )
    ...
    def _parse_ssh_section(self, config_set):
        ...
        ssh_section = self._find(
            config_set, "SSH", error_if_missing=False
        )
