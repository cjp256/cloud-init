...
class DataSourceAzure(sources.DataSource):
    ...
    @azure_ds_telemetry_reporter
    def get_imds_data_with_api_fallback(self) -> Dict:
        try:
    ...
    def _check_if_nic_is_primary(self, ifname):
        ...
        LOG.warning("Failed to get network metadata using nic %s.", ifname)
        self._teardown_ephemeral_networking()
        return False, -1
    ...
    def _poll_imds(self):
        ...
        while not reprovision_data:
            ...
            with events.ReportEventStack(
                ...
                try:
                    reprovision_data = imds.fetch_reprovisiondata()
                except UrlError as error:
                    report_diagnostic_event(
                        "poll IMDS with %s failed. "
                        "Exception: %s and code: %s"
                        % (error, error.cause, error.code),
                        logger_func=LOG.warning,
                    )
                    self._teardown_ephemeral_networking()
                    continue
